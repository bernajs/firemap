...
const objOnline = userOnline.child('realtime');
objOnline.set({status: 'connected', service: false});
objOnline
    .onDisconnect()
    .remove();
    ...
    var leadsRef = database
        ...
        .on('value', function (snapshot) {
            ...
            snapshot.forEach(function (childSnapshot) {
                ...
                // if (childData.realtime.servicio == 'pending') {
                //     if (confirm('Hay un nuevo servicio')) {
                //         aceptarServicio(childSnapshot.key, 'busy');
                //     } else {
                //         aceptarServicio(childSnapshot.key, 'connected');
                //     }
                // }
...
// (user) {         if (user) {             usuario.name = user.displayName;
//     usuario.email = user.email;             usuario.picture = 'google.com';
//           usuario.realtime = {                 status: 'connected',
//       servicio: false             }; getLocation();             if
// (navigator.geolocation) { navigator                     .geolocation
// .getCurrentPosition(function (position) {                         var pos = {
//                             lat: position.coords.latitude,          lng:
// position.coords.longitude                         };              usuario.pos
// = pos; localStorage.setItem('uid', user.uid);
// database                       .ref('users/' + user.uid) .set(usuario);
...
function solicitarServicio() {
    var leadsRef = database
        .ref('users')
        .orderByChild('realtime/status')
        .equalTo('connected')
        .once('value', function (snapshot) {
            snapshot
                .forEach(function (element) {
                    database
                        .ref('users/' + element.key + '/realtime/servicio')
                        .set('pending');
                });
        });
...
function aceptarServicio(key, status) {
    console.log(key);
    database
        .ref('users/' + key + '/realtime/')
        .set({servicio: true, status: status});
}
